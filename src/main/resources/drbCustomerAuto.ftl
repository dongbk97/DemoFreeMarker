<!DOCTYPE html>
<html>

<head>
    <meta http-equiv="Content-Type" content="text/html; charset=utf-8"></meta>
    <meta name="viewport" content="width=device-width, initial-scale=1.0"></meta>
    <title>Giấy nhận nợ</title>
    <style>
        @font-face {
            font-family: 'Roboto';
            src: url('${fontFamily}');
        }

        * {
            font-family: 'Roboto';
        }

        body {
            font-size: 12px;
        }

        .body td {
            border: 1px solid black
        }

        .d-none {
            display: none !important;
        }

        .text-left {
            text-align: left;
        }

        .text-center {
            text-align: center;
        }

        .text-right {
            text-align: right;
        }

        td {
            height: 30px;
            padding-bottom: 5px;
        }

        p {
            font-weight: bolder;
        }

        .ml-10 {
            margin-left: 10px;
        }

        .text-justify {
            text-align: justify;
            text-justify: inter-word;
        }

        #test-lanscape

        @page {
            size: A4 landscape;
        }

    </style>
</head>
<body>
<div style="margin: auto; width: 90%; display:block;">
    <table style="width: 100%; border-collapse: collapse; table-layout: fixed;">
        <colgroup>
        </colgroup>
        <tbody>
        <tr>
            <td colspan="12">
                <img style="height: 40px;"
                     src="data:image/jpeg;base64,"></img>
            </td>
            <td colspan="12">
                <p style="color: red; font-weight: bold;text-align: right; font-size: 0.8rem">
                    Ngân hàng TMCP Hàng Hải Việt Nam (MSB)
                </p>
                <p style="text-align: right; font-size: 0.8rem">
                    54A Nguyễn Chí Thanh, Quận Đống Đa, Hà Nội
                </p>
                <p style="text-align: right; font-size: 0.8rem">
                    Tel: (024) 3771 8989 - Fax: (024) 3771 8899
                </p>
            </td>
        </tr>
        <tr>
            <td colspan="24">
                <h2 style="font-weight: bold; text-align: center">GIẤY NHẬN NỢ</h2>
                <h4 style="font-weight: bold; text-align: center; font-style: italic;">Số: ${sugsNo} ,
                    Ngày: ${createdDate}</h4>
            </td>
        </tr>
        <tr>
            <td colspan="24">
                <h3 style="font-weight: bold;">A-ĐỀ NGHỊ GIẢI NGÂN</h3>
            </td>
        </tr>
        <tr>
            <td colspan="24">
                <p style="line-height: 20px; font-style: italic;">
                    Căn cứ Hợp đồng số ${contractNo} ${effectiveDate} giữa Ngân hàng TMCP Hàng hải Việt Nam
                    (MSB) và ${customerName} (KH) và các Phụ lục kèm theo;
                </p>
                <p style="line-height: 20px; font-style: italic;">
                    Căn cứ thỏa thuận về điều khoản và điều kiện sử dụng dịch vụ Giải ngân trực tuyến giữa
                    Ngân hàng TMCP Hàng hải Việt Nam (MSB) và ${customerName} (KH).
                </p>
            </td>
        </tr>
        <tr>
            <td style="border: 0.5px dashed black" colspan="13">
                <p style="margin-left: 10px;">
                    <b>Đơn vị nhận nợ: ${customerName}</b>
                </p>
            </td>
            <td style="border: 0.5px dashed black;" colspan="11">
                <p style="margin-left: 10px;">
                    Mã KH (CIF): ${cifNo}
                </p>
            </td>
        </tr>
        <tr style="border-top: 0.5px dashed black;">
            <td style="border: 0.5px dashed black" colspan="13" rowspan="2">
                <p style="margin-left: 10px;">
                    Giấy chứng nhận đăng ký doanh nghiệp/Quyết định thành lập số: ${customerIdNo}
                </p>
            </td>
            <td style="border: 0.5px dashed black; height: 30px;" colspan="11">
                <p style="margin-left: 10px;">
                    Do: ${issuePlace}
                </p>
            </td>
        </tr>
        <tr>
            <td style="border: 0.5px dashed black; height: 30px;" colspan="11">
                <p style="margin-left: 10px;">
                    cấp ngày: ${issueDate}
                </p>
            </td>
        </tr>
        <tr>
            <td style="border: 0.5px dashed black" colspan="11">
                <b style="margin-left: 10px;">Hạn mức giải ngân:</b>
            </td>
            <td style="border: 0.5px dashed black" colspan="13">
                <p style="margin-left: 10px;"> ${limitName}</p>
            </td>
        </tr>
        <tr>
            <td style="border: 0.5px dashed black" colspan="11">
                <b style="margin-left: 10px;">Mục đích giải ngân:</b>
            </td>
            <td style="border: 0.5px dashed black" colspan="13">
                <p style="margin-left: 10px;">Phục vụ sản xuất kinh doanh – ${disbursementPurpose}</p>
            </td>
        </tr>

        <tr>
            <td style="border: 0.5px dashed black" colspan="11">
                <b style="margin-left: 10px;">Ngày giải ngân:</b>
            </td>
            <td style="border: 0.5px dashed black" colspan="13">
                <p style="margin-left: 10px;">${createLoanSugHour} giờ ${createLoanSugMinute} phút
                    ngày ${createLoanSugDate}</p>
                <div class="ml-10">
                    <b>Thời điểm giải ngân thực tế có thể thực hiện trong ngày làm việc tiếp theo</b>
                </div>
            </td>
        </tr>
        <tr>
            <td style="border: 0.5px dashed black" colspan="11">
                <b style="margin-left: 10px;">Thời hạn vay:</b>
            </td>
            <td style="border: 0.5px dashed black" colspan="13">
                <p style="margin-left: 10px;"> ${loanTerm}</p>
            </td>
        </tr>
        <tr>
            <td style="border-top: 0.5px dashed black; border: 0.5px dashed black" colspan="11">
                <b style="margin-left: 10px;">Phương thức giải ngân:</b>
            </td>


            <td style="border-top: 0.5px dashed black; border: 0.5px dashed black" colspan="13">
                <div class=" ml-10" style="margin-top: 10px;">
                    <b style="font-family:Wingdings"></b>
                    <b> Bằng chuyển khoản</b>
                </div>
                <div class=" ml-10" style="margin-top: 10px;">
                    <b style="font-family:Wingdings"></b>
                    <b> Bằng tiền mặt</b>
                </div>
                <div class=" ml-10" style="margin-top: 10px;">

                    <b>Thông tin người thụ hưởng theo Chỉ dẫn thanh toán đính kèm</b>
                </div>
            </td>


        </tr>
        <tr>
            <td style="border: 0.5px dashed black" colspan="11">
                <b style="margin-left: 10px;">Số tiền giải ngân:</b>
            </td>
            <td style="border: 0.5px dashed black" colspan="13">
                <p style="margin-left: 10px;">Bằng số: ${numberAmountCus}</p>
                <p style="margin-left: 10px;">Bằng chữ: ${wordAmountCus}
                </p>
            </td>
        </tr>

        <tr>
            <td style="border-top: 0.5px dashed black; border: 0.5px dashed black" colspan="11">
                <p style="font-weight: bold; margin-left: 10px;">Các nội dung khác:</p>
            </td>
            <td style="border-top: 0.5px dashed black; border: 0.5px dashed black" colspan="13">
                <p style="margin-left: 10px;">
                    Để làm rõ, chúng tôi hiểu và xác nhận rằng, số tiền giải ngân và thời hạn vay cụ thể sẽ theo quyết
                    định
                    của MSB nhưng không vượt quá số tiền và thời hạn chúng tôi đã đề nghị.
                </p>
                <p style="margin-left: 10px;">
                    Các thông tin khác liên quan tới khoản vay như ngày giải ngân, lãi suất,
                    phí, lịch trả nợ…sẽ được xác định theo thông tin cụ thể tại Phần B
                    – Xác nhận kết quả giải ngân của MSB và tại các thông báo, văn bản có liên quan do MSB phát hành.
                </p>
                <p style="margin-left: 10px;">
                    Toàn bộ nội dung Giấy nhận nợ này có thể được tra cứu chi tiết tại chức năng "Quản lý giải ngân
                    - Truy vấn Danh sách giải ngân" trên hệ thống Internet Banking.
                </p>
            </td>
        </tr>
        </tbody>
    </table>
    <table style="margin: auto; width: 90%; display:block;">
        <colgroup>
        </colgroup>
        <tbody>
        <tr>
            <td colspan="24">
                <div class="text-justify">
                    <p style="margin-top: 5px;">&#9632; Chúng tôi cam kết:</p>
                    <ul style="list-style-type: '- '; line-height: 32px;">
                        <li>
                            Thanh toán vô điều kiện số tiền nhận nợ vay, tiền lãi, lãi phạt và các loại phí liên
                            quan cho MSB theo xác nhận kết quả giải ngân tại Giấy nhận nợ này;
                        </li>
                        <li>
                            Sử dụng tiền vay đúng mục đích, phù hợp với quy định của pháp luật và MSB;
                            <div>
                                Trường hợp theo chỉ dẫn của chúng tôi, khoản vay được giải ngân toàn bộ hay một phần
                                bằng tiền mặt
                                hoặc chuyển khoản vào tài khoản của chúng tôi, thì chúng tôi cam kết sẽ chuyển khoản
                                tiền này
                                là phù hợp với quy định của pháp luật và sẽ được chúng tôi chuyển đến các bên thụ hưởng
                                có liên quan
                                trong ngày giải ngân hoặc ngày làm việc kế tiếp ngày giải ngân, và sẽ nộp chứng từ chứng
                                minh
                                việc chuyển khoản tiền này cho các bên thụ hưởng cho MSB theo đúng yêu cầu tại Phần
                                B-Xác nhận kết quả giải ngân;
                            </div>
                        </li>
                        <li>
                            Việc giải ngân bằng tiền mặt chỉ áp dụng trong trường hợp Bên thụ hưởng (không bao gồm pháp
                            nhân)
                            không có tài khoản thanh toán tại tổ chức cung ứng dịch vụ thanh toán và với các khoản giải
                            ngân tiền mặt
                            như vậy, chúng tôi cam kết rằng chúng tôi đã kiểm soát và bảo đảm rằng Bên thụ hưởng (không
                            bao gồm pháp nhân)
                            không có tài khoản thanh toán tại bất cứ tổ chức cung ứng dịch vụ thanh toán nào;
                        </li>
                        <li>
                            Nếu vốn vay được giải ngân bằng ngoại tệ theo các trường hợp sau, chúng tôi cam kết tuân thủ
                            quy định
                            về ngoại hối của NHNN Việt Nam:

                            <ul style="list-style-type: square;">

                                <li>
                                    Vốn vay được dùng để thanh toán ra nước ngoài tiền nhập khẩu hàng hóa,
                                    dịch vụ/để đáp ứng các nhu cầu vốn trong nước nhằm thực hiện phương án sản xuất,
                                    kinh doanh hàng hóa xuất khẩu qua cửa khẩu biên giới Việt Nam và chúng tôi
                                    có đủ ngoại tệ từ nguồn thu sản xuất, kinh doanh để trả nợ vay theo quy định của
                                    pháp luật hiện hành, hoặc;
                                </li>
                                <li>
                                    Vốn vay được dùng để đáp ứng các nhu cầu vốn trong nước nhằm thực hiện phương án sản
                                    xuất,
                                    kinh doanh hàng hóa xuất khẩu qua cửa khẩu biên giới Việt Nam và chúng tôi có đủ
                                    ngoại tệ
                                    từ nguồn thu xuất khẩu để trả nợ vay theo quy định của pháp luật hiện hành;
                                </li>

                            </ul>

                        </li>
                        <li>
                            Các hồ sơ, chứng từ cung cấp cho MSB để nhận nợ là hoàn toàn hợp lệ, chính xác,
                            trung thực, đầy đủ và chúng tôi chịu mọi trách nhiệm đối với các rủi ro, tổn thất phát sinh
                            nếu có;
                        </li>
                        <li>
                            Đồng ý với việc chuyển đổi sản phẩm vay theo thông báo của MSB trên cơ sở các thỏa thuận cho
                            vay đã ký nếu có;
                        </li>

                    </ul>
                    <p style="line-height: 32px;">&#9632;
                        Giấy nhận nợ này gồm 2 phần, phần A-Đề nghị giải ngân và
                        phần B- Xác nhận kết quả giải ngân. Trường hợp có sự khác
                        biệt giữa Phần A và Phần B của Giấy nhận nợ này thì các bên
                        thống nhất áp dụng theo các nội dung nêu tại Phần B - Xác
                        nhận kết quả giải ngân. Giấy nhận nợ này có thể được các bên
                        sửa đổi, bổ sung hoặc hủy bỏ bằng văn bản thỏa thuận cụ thể.
                    </p>
                    <p style="line-height: 32px;">&#9632; Giấy nhận nợ này là bộ phận gắn liền, không tách rời của <b>Hợp
                            đồng số: ${contractNo} ${effectiveDate} , Điều khoản và điều kiện sử dụng dịch vụ Giải
                            ngân trực tuyến và văn bản thỏa thuận tham gia sản phẩm giữa MSB và KH (trường hợp KH có sử
                            dụng sản phẩm).</b>
                    </p>
                </div>
            </td>
        </tr>
        <tr>
            <td colspan="2"></td>
            <td colspan="16">
                <p style="font-weight: bold;">ĐẠI DIỆN ĐƠN VỊ NHẬN NỢ</p>
            </td>
        </tr>
        <tr>
            <td colspan="1"></td>
            <td colspan="8">
                <b style="line-height: 32px;">Tên người đại diện</b>
            </td>
            <td colspan="8">
                <b>: ${customerIbName}</b>
            </td>
        </tr>
        <tr>
            <td colspan="1"></td>
            <td colspan="8">
                <b style="line-height: 32px;">Mã người dùng<br/> trên Internet Banking</b>
            </td>
            <td colspan="8">
                <b>: ${username}</b>
            </td>
        </tr>
        <tr>
            <td colspan="1"></td>
            <td colspan="8">
                <b style="line-height: 32px;">Số CMND/Hộ chiếu/CCCD</b>
            </td>
            <td colspan="8">
                <b>: ${cusCertCode}</b>
            </td>
        </tr>
        </tbody>
    </table>
</div>

<!-- Trang 2 -->
<div style="margin-top: 150px; font-style: italic;">
    (*) Quý khách lưu ý: Thông tin Giấy nhận nợ được in ra từ dịch vụ trực tuyến của MSB và được xác thực bằng chữ ký
    số.
    <br/>
    Trường hợp Quý khách muốn chuyển đổi Giấy nhận nợ từ chứng từ điện tử sang chứng từ giấy, vui lòng liên hệ với chi
    nhánh MSB đang hỗ trợ giao dịch của Quý khách.
</div>
<!-- Trang 3 -->

<div id="test-lanscape" class="${paymentDomestic}" style="margin: auto; width: 90%; display:block; page-break-before:always;@page {
            size: A4 landscape;
        }">
    <table style="width: 100%; border-collapse: collapse; table-layout: fixed;">
        <colgroup>
            <col span="24"></col>
        </colgroup>
        <tbody>
        <tr>
            <td colspan="24">
                <h2 style="font-weight: bold; text-align: center">
                    CHỈ DẪN THANH TOÁN CHUYỂN TIỀN QUỐC TẾ
                </h2>
            </td>
        </tr>
        <tr>
            <td colspan="24">
                <h4 style="text-align: center; font-style: italic;">(Đính kèm Giấy nhận nợ số: ${sugsNo} - Phần A: Đề
                    nghị giải ngân)</h4>
            </td>
        </tr>
        <tr>
            <td colspan="24">
                <b>I. Thông tin người chuyển:</b>
            </td>
        </tr>
        <tr>
            <td colspan="24">
                <ul style="list-style-type: '- '; line-height: 32px;">
                    <li>Tên người chuyển: ${customerNamePay}</li>
                    <li>Mã số thuế: ${taxCode} </li>
                    <li>Tổng số tiền chuyển: ${totalAmoutApprove}</li>
                    <li>Ngày chuyển: ${approvalLoanDate}</li>
                </ul>
            </td>
        </tr>
        <tr>
            <td colspan="24">
                <b>II. Thông tin người thụ hưởng:</b>
            </td>
        </tr>
        <tr>
            <td colspan="24">
                <ul style="list-style-type: '- '; line-height: 32px;">
                    <li>Chi tiết theo lệnh chỉ dẫn chuyển tiền quốc tế đính kèm số: ${customerNamePay}</li>
                </ul>
            </td>
        </tr>

        <tr>
            <td colspan="2"></td>
            <td colspan="16">
                <p style="font-weight: bold;">ĐẠI DIỆN KHÁCH HÀNG</p>
            </td>
        </tr>
        <tr>
            <td colspan="1"></td>
            <td colspan="8">
                <b style="line-height: 32px;">Tên người đại diện</b>
            </td>
            <td colspan="8">
                <b>: ${customerIbName}</b>
            </td>
        </tr>
        <tr>
            <td colspan="1"></td>
            <td colspan="8">
                <b style="line-height: 32px;">Mã người dùng<br/> trên Internet Banking</b>
            </td>
            <td colspan="8">
                <b>: ${username}</b>
            </td>
        </tr>
        <tr>
            <td colspan="1"></td>
            <td colspan="8">
                <b style="line-height: 32px;">Số CMND/Hộ chiếu/CCCD</b>
            </td>
            <td colspan="8">
                <b>: ${cusCertCode}</b>
            </td>
        </tr>

        </tbody>
    </table>
    <br/>

</div>
<!-- Trang 4 -->

<!-- Trang 4 -->

<!-- Trang 5 -->

<#--<div class="${convertDrbDocId} &lt;#&ndash;d-none&ndash;&gt;">-->
<#--    <div style="margin: auto; width: 90%; display:block; page-break-before:always;">-->
<#--        <h2 style="font-weight: bold; text-align: center">CHUYỂN ĐỔI GIẤY NHẬN NỢ TỪ CHỨNG TỪ ĐIỆN TỬ SANG CHỨNG TỪ-->
<#--            GIẤY</h2>-->
<#--        <table style="width: 100%; border-collapse: collapse; table-layout: fixed;">-->
<#--            <colgroup>-->
<#--                <col span="24"></col>-->
<#--            </colgroup>-->
<#--            <tbody>-->
<#--            <tr>-->
<#--                <td colspan="12">-->
<#--                    <p style="font-weight: bold; text-align: center">-->
<#--                        Người thực hiện chuyển đổi-->
<#--                    </p>-->
<#--                    <p style="text-align: center; margin-bottom: 100px;">-->
<#--                        (Ký, ghi rõ họ tên)-->
<#--                    </p>-->
<#--                </td>-->
<#--                <td colspan="12">-->
<#--                    <p style="text-align: center">-->
<#--                        Thời gian chuyển đổi:-->
<#--                    </p>-->
<#--                    <p style="text-align: center; margin-bottom: 100px;">-->
<#--                        ${convertDrbDoc}-->
<#--                    </p>-->
<#--                </td>-->
<#--            </tr>-->
<#--            <tr>-->
<#--                <td colspan="12">-->
<#--                    <p style="font-weight: bold; text-align: center">-->
<#--                        ĐẠI DIỆN ĐƠN VỊ NHẬN NỢ-->
<#--                    </p>-->
<#--                    <p style="text-align: center">-->
<#--                        (Ký, ghi rõ họ tên, chức danh và đóng dấu)-->
<#--                    </p>-->
<#--                </td>-->
<#--                <td colspan="12">-->
<#--                    <p style="font-weight: bold; text-align: center">-->
<#--                        ĐẠI DIỆN MSB-->
<#--                    </p>-->
<#--                    <p style="text-align: center">-->
<#--                        (Ký, ghi rõ họ tên, chức danh và đóng dấu)-->
<#--                    </p>-->
<#--                </td>-->

<#--            </tr>-->
<#--            </tbody>-->
<#--        </table>-->
<#--    </div>-->
<#--</div>-->

</body>

</html>